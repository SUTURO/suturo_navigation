...
from obstacle_finder.cfg import ObstacleFinderConfig
...
class ObstacleFinder:
    ...
    def drc_callback(self, config, level):
    	print(config)
    	self._occ_threshold = config.occ_threshold
        self._min_scans_cluster = config.min_scans_cluster
        self._min_percentage_covered = config.min_percentage_covered
        self._rad_neighbour = config.rad_neighbour
        self._error_map = config.error_map
    	return config
...
if __name__ == '__main__':
    ...
    of = ObstacleFinder()
    srv = Server(ObstacleFinderConfig, of.drc_callback)
    rospy.spin()
